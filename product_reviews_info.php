<?php
/* -----------------------------------------------------------------------------------------
   $Id: product_reviews_info.php 4200 2013-01-10 19:47:11Z Tomcraft1980 $
...
$smarty->assign('RATING', xtc_image('templates/'.CURRENT_TEMPLATE.'/img/stars_'.$reviews['reviews_rating'].'.gif', sprintf(TEXT_OF_5_STARS, $reviews['reviews_rating']),'','','itemprop="rating"'));
...
$review_image = DIR_WS_THUMBNAIL_IMAGES.$reviews['products_image'];
if(!file_exists($review_image)) $review_image = DIR_WS_THUMBNAIL_IMAGES.'noimage.gif'; //Tomcraft - 2009-10-30 - noimage.gif is displayed, when no image is defined
$smarty->assign('IMAGE', '<a href="javascript:popupImageWindow(\''.xtc_href_link(FILENAME_POPUP_IMAGE, 'pID='.$reviews['products_id']).'\')">'.xtc_image($review_image, $reviews['products_name'], '', '', 'align="center" hspace="5" vspace="5"').'<br /></a>');
