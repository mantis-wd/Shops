<?php
  /* -----------------------------------------------------------------------------------------
   $Id: gunnart_productRedirect.inc.php 2522 2011-12-14 13:45:11Z dokuman $
   -----------------------------------------------------------------------------------------*/
  // -----------------------------------------------------------------------------------------
  ...
  function ProductRedirectionLink($ProdID=false) {
    if($ProdID) {
      ...
      $dbQuery = xtDBquery("SELECT p.products_id, 
                                   pd.products_name
                              FROM ".TABLE_PRODUCTS_DESCRIPTION." pd,
                                   ".TABLE_PRODUCTS." p
                             WHERE pd.products_id = '".(int)$ProdID."'
      ...
      if(!empty($dbQuery['products_id'])) {
        return xtc_href_link(FILENAME_PRODUCT_INFO,xtc_product_link((int)$ProdID,$dbQuery['products_name']));
      }
    }
    return false;
  }
  ...
  function productRedirect() {
    ...
    if(basename($_SERVER['SCRIPT_NAME']) == FILENAME_PRODUCT_INFO) {
      ...
      $RedirectionLink = ProductRedirectionLink((int)$actual_products_id);
