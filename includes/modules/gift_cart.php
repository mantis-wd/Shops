<?php
/* -----------------------------------------------------------------------------------------
   $Id: gift_cart.php 4200 2013-01-10 19:47:11Z Tomcraft1980 $
...
$gift_smarty->caching = false; //DokuMan - 2012-10-30 - avoid Smarty caching in order to display the correct data, if caching is enabled in shop backend

$gift_smarty->assign('tpl_path',DIR_WS_BASE.'templates/'.CURRENT_TEMPLATE.'/');
$gift_smarty->assign('ACTIVATE_GIFT', 'true');

if (isset ($_SESSION['customer_id'])) {
  $gv_query = xtc_db_query("-- /includes/modules/gift_cart.php
                            SELECT amount
...
if (isset ($_SESSION['cc_id'])) { 
  ...
  $gift_smarty->assign('COUPON_HELP_LINK', xtc_href_link(FILENAME_POPUP_COUPON_HELP, 'cID='.$_SESSION['cc_id'].POPUP_COUPON_HELP_LINK_PARAMETERS, $request_type));
}
if (isset ($_SESSION['customer_id'])) {
