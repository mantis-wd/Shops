...
/**************************************************************/
  function create_paypal_link() {
    ...
    $data_link = $href .'&amp;business='.$business.'&amp;item_name='. $store_name. $item_number .'&amp;item_number='.$item_number.'&amp;amount='. ($amount - $shipping).'&amp;shipping='.$shipping.'&amp;currency_code='.$currency_code.'&amp;return='. $return .'&amp;no_shipping='. $no_shipping .'&amp;cbt='. $cbt .'&amp;cancel_return='. $cancel_return. '&amp;cpp_headerback_color='. $cpp_headerback_color.'&amp;cpp_headerborder_color='.$cpp_headerborder_color.'&amp;cpp_payflow_color='.$cpp_payflow_color.'&amp;cpp_header_image='.$cpp_header_image.'&amp;cbt='.$cbt.'&amp;lc='.$lc.'&amp;buttonsource='.MODULE_PAYMENT_PAYPAL_IPN_API_KEY;
...
/**************************************************************/
  function install() {
    ...
    xtc_db_query("insert into ".TABLE_CONFIGURATION." ( configuration_key, configuration_value,  configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYPAL_IPN_API_KEY', '120,116,99,77,111,100,105,102,105,101,100,95,67,97,114,116,95,69,67,77', '6', '5', now())");
    ...
    // BOF - Hendrik - 2010-08-11 - exlusion config for shipping modules
    xtc_db_query("insert into ".TABLE_CONFIGURATION." ( configuration_key, configuration_value,  configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PAYPAL_IPN_NEG_SHIPPING', '', '6', '99', now())");
    // EOF - Hendrik - 2010-08-11 - exlusion config for shipping modules
  }
/**************************************************************/
  function remove() {
