<?php
/*
  $Id: iclear_prepayment.php 2123 2011-08-29 10:08:43Z dokuman $
...
class iclear_prepayment extends iclear {
	
	function iclear_prepayment ( ) {
		parent::iclear( 'prepayment' );
		$this->code = 'iclear_prepayment';
		$this->description =  $this->lang->getParam('DESCRIPTION_PREPAYMENT');
		$this->enabled = $this->proxy->perform('enabled');
	}
	
	function selection() {
		$res = parent::selection( );
		$res['id'] = 'iclear_prepayment';
		$res['module'] = $this->proxy->perform('title');
		$res['description'] = $this->lang->getParam('INFO_EXTENDED_PREPAYMENT');
		return $res;
	}
