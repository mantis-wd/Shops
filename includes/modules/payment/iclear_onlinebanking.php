<?php
/*
  $Id: iclear_onlinebanking.php 2123 2011-08-29 10:08:43Z dokuman $
...
class iclear_onlinebanking extends iclear {
	
	function iclear_onlinebanking ( ) {
		parent::iclear( 'onlinebanking' );
		$this->code = 'iclear_onlinebanking';
		$this->description =  $this->lang->getParam('DESCRIPTION_ONLINEBANKING');
		$this->enabled = $this->proxy->perform('enabled');
	}
	
	function selection() {
		$res = parent::selection( );
		$res['id'] = 'iclear_onlinebanking';
		$res['module'] = $this->proxy->perform('title');
		$res['description'] = $this->lang->getParam('INFO_EXTENDED_ONLINEBANKING');
		return $res;
	}
