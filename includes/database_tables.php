<?php
/* -----------------------------------------------------------------------------------------
   $Id: database_tables.php 4200 2013-01-10 19:47:11Z Tomcraft1980 $
...
define('TABLE_SHOP_CONFIGURATION', 'shop_configuration');
...
foreach ($constants['user'] AS $k=>$v) {
  if (strpos($k, 'TABLE_') !== 0)
    continue;

  $tmp = explode('_', $v);
  $str = '';
  foreach($tmp AS $t)
    if (strlen($t)) // This is needed because of TABLE_PERSONAL_OFFERS_BY
      $str .= $t[0].$t[1];

  $counter = 0;
  while (in_array($str.$counter, $tablesArray))
    $counter++;
