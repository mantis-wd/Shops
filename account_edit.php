<?php
/* -----------------------------------------------------------------------------------------
   $Id: account_edit.php 4200 2013-01-10 19:47:11Z Tomcraft1980 $
...
// set $customers_status separately, as it is required for changing the customers status (e.g. to merchant)
...
if (isset ($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  $csid = xtc_db_prepare_input($_POST['csid']);
  if (ACCOUNT_DOB == 'true') {
...
2), substr(xtc_date_raw($dob), 6, 2), substr(xtc_date_raw($dob), 0, 4)) == false)) {
  ...
  // check for emails in account_edit
  } else {
  ...
  // confirm e-mail at registration
  ...
  if ($error == false) {
    ...
    'customers_status' => $customers_status, //DokuMan - 2011-12-01 - set $customers_status separately, as it is required for changing the customers status (e.g. to merchant)
    ...
    // prevent e_notice - init $account on error
    if (ACCOUNT_GENDER == 'true') {
      $account['customers_gender'] = $gender;
    }
    if (ACCOUNT_COMPANY_VAT_CHECK == 'true') {
      $account['customers_vat_id'] = $vat;
    }
    $account['customers_firstname'] = $firstname;
    $account['customers_lastname'] = $lastname;
    $account['customers_cid'] = $csid;
    if (ACCOUNT_DOB == 'true') {
      $account['customers_dob'] = xtc_date_raw($dob);
    }
    $account['customers_email_address'] = $email_address;
    $account['customers_telephone'] = $telephone;
    $account['customers_fax'] = $fax;
  }
} else {
...
$smarty->assign('FORM_ACTION', xtc_draw_form('account_edit', xtc_href_link(FILENAME_ACCOUNT_EDIT, '', 'SSL'), 'post', 'onsubmit="return check_form(account_edit);"').xtc_draw_hidden_field('action', 'process').xtc_draw_hidden_field('csid', $account['customers_cid']));
