<?php
  /* --------------------------------------------------------------
   $Id: mail.php 4200 2013-01-10 19:47:11Z Tomcraft1980 $
   ...
   (c) 2006 XT-Commerce (mail.php 955 2005-05-19)
  ...
  if (in_array($action, array('preview', 'send_email_to_user')) && ($_POST['customers_email_address']) && !array_key_exists('back', $_POST)) { //DokuMan - 2011-08-31 - do not send an email if admin cancels it
    ...
    switch ($_POST['customers_email_address']) {
      ...
      default:
        if (is_numeric($_POST['customers_email_address'])) {
          $mail_query = xtc_db_query("select customers_firstname, customers_lastname, customers_email_address from " . TABLE_CUSTOMERS . " where customers_status = " . (int)$_POST['customers_email_address']);
          $sent_to_query = xtc_db_query("select customers_status_name from " . TABLE_CUSTOMERS_STATUS . " WHERE customers_status_id = '" . (int)$_POST['customers_email_address'] . "' AND language_id='" . (inT)$_SESSION['languages_id'] . "'");
          $row = xtc_db_fetch_array($sent_to_query);
          $mail_sent_to = $row['customers_status_name'];
        } else {
  ...
  if ( ($action == 'preview') && (!$_POST['customers_email_address']) ) {
...
require (DIR_WS_INCLUDES.'head.php');
  ...
  if (USE_WYSIWYG=='true' && empty($action)) { //display WYSIWYG only if there's no action 
                        ...
                        <td>
                          <table width="70%" border="0" cellpadding="0" cellspacing="2">
                            ...
                            <tr>
                              ...
                              <td><?php echo xtc_draw_textarea_field('message', 'soft', '100%', '30',isset($_POST['message'])?$_POST['message']:''); ?></td>
                            </tr>
                            <tr>
                              <td colspan="2"><?php echo xtc_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                            </tr>
                            <tr>
                              <td colspan="2" align="center"><input type="submit" class="button" value="<?php echo BUTTON_SEND_EMAIL; ?>"></td>
                            </tr>
                          </table>
                        </td>
                      </form>
