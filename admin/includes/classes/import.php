<?php
  /* -----------------------------------------------------------------------------------------
   $Id: import.php 4200 2013-01-10 19:47:11Z Tomcraft1980 $
  ...
  class xtcImport {
    ...
    function map_file($mapping) {
      if (!file_exists($this->ImportDir.$this->filename)) {
        ...
        $inhalt = array ();
        $inhalt = file($this->ImportDir.$this->filename, FILE_IGNORE_NEW_LINES);
    ...
    *****************************************************************************/
    ...
    function insertProduct(& $dataArray, $mode = 'insert',$touchCat = false) {
      ...
      if ($this->FileSheme['p_priceNoTax'] == 'Y')
        $products_array = array_merge($products_array, array ('products_price' => str_replace(',', '.', $dataArray['p_priceNoTax'])));
        ...
        for ($i = 0; $i < count($cat); $i ++) {
          ...
          if (is_int($ID) || $ID == '0') {
            ...
            $cat_query = xtc_db_query("SELECT c.categories_id
                                        ...
                                        WHERE cd.categories_name='".addslashes($cat[$i])."'
  ...
  class xtcExport {
    ...
    function exportProdFile() {
      ...
      // To export only active products, use the following SELECT statement instead of the above
      //                            SELECT * FROM '.TABLE_PRODUCTS.' WHERE products_status='1');
        ...
        while ($this->getParent($tmpID) != 0 || $tmpID != 0) {
          $sql = '-- admin/includes/classes/import export buildCat
                  SELECT categories_name FROM '.TABLE_CATEGORIES_DESCRIPTION.'
                                        WHERE categories_id='.$tmpID.'
                                          AND language_id='.$this->languages[0]['id'];
