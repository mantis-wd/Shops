<?php
  /* --------------------------------------------------------------
   $Id: categories.php 4563 2013-04-02 15:08:45Z Tomcraft1980 $
...
//###############################//
  ...
  class categories {
    ...
    function set_category_recursive($categories_id, $status = "0", $on_products = false, $cat_depth = 0) {

      $cats_list = '';
      $left_join = '';

      // set status till categories depth
      if ($cat_depth) $depth = $cat_depth;
      elseif (!defined('CATEGORIES_MAX_DEPTH')) $depth = 4;
      else $depth = CATEGORIES_MAX_DEPTH;

      // extend query for categories depth
      if ($depth > 1) {
        for($c = 2; $c <= $depth; $c++) {
          $cats_list .= ' ,c'.$c.'.categories_id';
          $left_join  .= ' LEFT JOIN categories c'.$c.' ON c'.($c-1).'.categories_id = c'.$c.'.parent_id';
        }
      }

      // set status of products in category
      if ($on_products) {
        xtc_db_query("UPDATE ".TABLE_PRODUCTS."
                         SET products_status=".$status."
                       WHERE products_id IN
                             (SELECT DISTINCT p2c.products_id
                                FROM products_to_categories p2c
                                JOIN
                                     (SELECT concat_ws(',', c1.categories_id".$cats_list.") str
                                     FROM categories c1 ".$left_join."
                                     WHERE c1.categories_id = ".$categories_id.") s
                                  ON concat(',', s.str, ',') LIKE concat('%,', p2c.categories_id, ',%')");
      }

      // set status of categories with sub categories
      xtc_db_query("UPDATE categories
      ...
      /* old method
    // set status on products in category
    if ($on_products) {
      ...
      while ($categories = xtc_db_fetch_array($categories_query)) {
        $this->set_category_recursive($categories['categories_id'], $status, $on_products);
      }
   */
    }
    ...
    function insert_product($products_data, $dest_category_id, $action = 'insert') {
      ...
      $products_data['products_price'] = xtc_convert_value($products_data['products_price']);

      //BOF - Dokuman - 2012-06-05 - replaces  "," with "." - thx to pq
      ...
      //EOF - Dokuman - 2012-06-05 - replaces  "," with "." - thx to pq
      ...
      $products_data['products_discount_allowed'] = xtc_convert_value($products_data['products_discount_allowed']);
      $products_data['products_weight'] = xtc_convert_value($products_data['products_weight']);
      ...
      for ($col = 0, $n = sizeof($group_data); $col < $n +1; $col ++) {
        if ($group_data[$col]['STATUS_ID'] != '') {
          $products_data['products_price_'.$group_data[$col]['STATUS_ID']] = xtc_convert_value($products_data['products_price_'.$group_data[$col]['STATUS_ID']]);
      ...
      for ($col = 0, $n = sizeof($group_data); $col < $n +1; $col ++) {
        if ($group_data[$col]['STATUS_ID'] != '') {
          ...
          $products_data['products_price_staffel_'.$group_data[$col]['STATUS_ID']] = xtc_convert_value($products_data['products_price_staffel_'.$group_data[$col]['STATUS_ID']]);
