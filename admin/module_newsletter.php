<?php
  /* --------------------------------------------------------------
   $Id: module_newsletter.php 4200 2013-01-10 19:47:11Z Tomcraft1980 $
        ...
        if ($error == false) {
          ...
          for ($i=0,$n=sizeof($groups);$i<$n;$i++) {
            ...
            if (xtc_db_prepare_input($_POST['status_all'])=='yes') {
              $customers_query=xtc_db_query("-- admin/module_newsletter
                                             SELECT customers_id,
                                                    customers_firstname,
                                                    customers_lastname,
                                                    customers_email_address
                                               FROM ".TABLE_CUSTOMERS."
                                              WHERE customers_status='".(int)$groups[$i]."'");
            } else {
              $customers_query=xtc_db_query("-- admin/module_newsletter
  ...
  if (isset($_GET['send'])) {
    ...
    $newsletters_query=xtc_db_query("-- admin/module_newsletter
                                     SELECT title,
                                            body,
                                            bc,
                                            cc
    ...
    //Send an email to CC newsletter receiver (if CC is set)
    ...
    if ($break=='1') {
      // finished
      $limit1_query=xtc_db_query("-- admin/module_newsletter
                                  SELECT count(*) as count
                                    FROM module_newsletter_temp_".(int)$_GET['ID']."
                                   WHERE comment='send'");
      ...
      if ($limit1_data['count']-$limit_data['count']<=0) {
        ...
        $messageStack->add_session(sprintf(INFO_NEWSLETTER_SEND, $limit1_data['count']), 'success');
        xtc_redirect(xtc_href_link(FILENAME_MODULE_NEWSLETTER));
      } else {
        $messageStack->add_session(sprintf(INFO_NEWSLETTER_SEND, $limit1_data['count']), 'success');
        $messageStack->add(sprintf(INFO_NEWSLETTER_SEND.'<br />'.INFO_NEWSLETTER_LEFT, $limit1_data['count']-$limit_data['count']), 'error');
                    ...
                    <td>
                      ...
                      switch ($action) {
                        default:
                          ...
                          $customer_group_query=xtc_db_query("-- admin/module_newsletter
                          ...
                          while ($customer_group_data=xtc_db_fetch_array($customer_group_query)) {
                            // get single users
                            $group_query=xtc_db_query("-- admin/module_newsletter
                                                       SELECT count(*) as count
                                                         FROM ".TABLE_NEWSLETTER_RECIPIENTS."
                                                        WHERE mail_status='1'
                                                          AND customers_status='".(int)$customer_group_data['customers_status_id']."'");
                          ...
                          $newsletters_query=xtc_db_query("-- admin/module_newsletter
                                                           SELECT newsletter_id,date,title
                                                             FROM ".TABLE_MODULE_NEWSLETTER."
                                                            WHERE status='0'");
                          $news_data=array();
                            ...
                            for ($i=0,$n=sizeof($news_data); $i<$n; $i++) {
                              if ($news_data[$i]['id']!='') {
                                ...
                                if (isset($_GET['ID']) && $_GET['ID']!='' && $_GET['ID']==$news_data[$i]['id']) {
                                  $total_query=xtc_db_query("-- admin/module_newsletter
                                                             SELECT count(*) as count
                                                               FROM module_newsletter_temp_".(int)$_GET['ID']."");
                                  ...
                                  for ($i=0,$n=sizeof($customers_status);$i<$n; $i++) {
                                    $group_query=xtc_db_query("-- admin/module_newsletter
                                                               SELECT count(*) as count
                                                                 FROM ".TABLE_NEWSLETTER_RECIPIENTS."
                                                                WHERE mail_status='1'
                                                                  AND customers_status='".(int)$customers_status[$i]['id']."'");
                                    $group_data=xtc_db_fetch_array($group_query);
                                    $group_query=xtc_db_query("-- admin/module_newsletter
                                                               SELECT count(*) as count
                                                                 FROM ".TABLE_CUSTOMERS."
                                                                WHERE customers_status='".(int)$customers_status[$i]['id']."'");
